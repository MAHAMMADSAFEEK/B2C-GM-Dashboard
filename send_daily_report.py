...
# ── Single source of truth for module rates ──────────────────────────
# Rates are NOT maintained here. They are read at runtime from the dashboard
# HTML (index*.html) so the email always matches exactly what the dashboard
# shows. Update rates in the dashboard only; the email follows automatically.
import re as _re

def _load_rates_from_dashboard():
    """Parse MODULE_RATE_CUTOFF and MODULE_RATE_TABLE out of the dashboard
    index*.html that lives alongside this script. Returns (cutoff, table)."""
    ...
    for path in candidates:
        try:
            ...
            continue
        cut_m = _re.search(r"MODULE_RATE_CUTOFF\s*=\s*'([^']+)'", html)
        tbl_m = _re.search(r"MODULE_RATE_TABLE\s*=\s*\{(.*?)\}", html, _re.S)
        if not tbl_m:
            continue
        table = {}
        for k, v in _re.findall(r"'((?:[^'\\]|\\.)*)'\s*:\s*([0-9.]+)", tbl_m.group(1)):
            table[k.replace("\\'", "'")] = float(v)
        if table:
            cutoff = cut_m.group(1) if cut_m else '2026-04-01'
            print('[rates] loaded {} module rates (cutoff {}) from {}'.format(
                len(table), cutoff, os.path.basename(path)))
            return cutoff, table
    raise RuntimeError(
        'Could not find MODULE_RATE_TABLE in any index*.html next to this '
        'script. Place the dashboard HTML in the same folder so the email '
        'uses the same rates as the dashboard.')

MODULE_RATE_CUTOFF, MODULE_RATE_TABLE = _load_rates_from_dashboard()
_MODULE_RATE_LC = {k.lower().strip(): v for k, v in MODULE_RATE_TABLE.items()}

def _get_module_override_rate(module_type, dt_str):
    if not module_type: return None
    if dt_str and dt_str < MODULE_RATE_CUTOFF: return None
    key = module_type.strip().lower()
    return _MODULE_RATE_LC.get(key)

def _apply_rate_overrides(projects):
    """Apply module rate overrides to projects list (matches dashboard logic)."""
    for p in projects:
        mt = (p.get('mt') or '').strip()
        mq = p.get('mq') or 0
        dt = p.get('dt') or ''
        if mt and mq > 0:
            rate = _get_module_override_rate(mt, dt)
            if rate is not None:
                old_mod = p.get('mod', 0)
                new_mod = rate * mq
                p['mod'] = new_mod
                p['cogs'] = p['cogs'] - old_mod + new_mod

def load_data():
    with gzip.open(DATA_FILE, 'rt', encoding='utf-8') as f:
        raw = json.load(f)
    if isinstance(raw, dict) and 'projects' in raw:
        _apply_rate_overrides(raw['projects'])
    elif isinstance(raw, list):
        _apply_rate_overrides(raw)
    return raw
...
def build_sku_html(sku_data, aos_d, prev_lbl, curr_lbl, main_kw_c=None, main_kw_p=None, pj_cat=None):
    ...
    # Module: use vendor rate per panel Wp from MODULE_RATE_TABLE (matches dashboard)
    # DN dump has 1 row per project (not per panel), so we can't compute qty from it.
    # Instead, use the rate table rate directly: rwp = rate / panel_wattage.
    # This is the actual vendor rate and stays stable month-to-month (correct behavior).
    def agg_module(bucket):
        costs=defaultdict(float); counts=defaultdict(int)
        for p in bucket.values():
            for i in p['items']:
                if i['cat'] != 'Module': continue
                costs[i['item']] += i['amt']
                counts[i['item']] += 1
        tc = sum(costs.values())
        result = {}
        for name in costs:
            # Use rate table for Rate/Wp (same as dashboard MODULE_RATE_TABLE)
            rate = _get_module_override_rate(name, '2026-06-01')  # past cutoff
            try: pw = float(name.strip().split()[0])
            except: pw = 0
            if rate and pw > 0:
                rwp = rate / pw  # vendor rate per panel watt
            elif pw > 0:
                # Fallback: cost / (count × kw_avg × 1000) — approximate
                rwp = costs[name] / sum(p['kw']*1000 for p in bucket.values()
                                        for i in p['items'] if i['item']==name) if costs[name] else 0
            else:
                rwp = 0
            result[name] = {'cost':costs[name], 'rwp':rwp,
                            'mix':costs[name]/tc*100 if tc else 0}
        return result, 0, tc
    a_mod,_,_ = agg_module(curr_p)
    p_mod,_,_ = agg_module(prev_p)
...
.kc-label{
  ...
  text-transform:none;color:#475569;margin-bottom:5px;
...
.kc-insight{
  display:block;font-size:8.5px;color:#0369A1;
  margin-top:6px;line-height:1.45;
  white-space:normal;font-style:italic;
  border-top:1px dashed #BFDBFE;padding-top:5px;
}
.kc-sub{
...
/* Mobile cluster cards — hidden on desktop */
...
@media screen and (max-width:600px){
  body{padding:8px 4px 32px;font-size:13px}
  .page{max-width:100%}
  .header{padding:16px 12px 12px;border-radius:12px 12px 0 0}
  .header h1{font-size:15px;line-height:1.3}
  .header-meta{font-size:9px}
  .eyebrow{font-size:9px}
  .badge{font-size:9px;padding:3px 7px;white-space:normal}

  /* SKU grid — single column */
  .sku-grid{grid-template-columns:1fr!important;gap:8px}
  .sku-card{padding:10px 11px}
  .sku-line{font-size:10px;line-height:1.9}

  /* Hide desktop cluster table, show cards */
  .cluster-wrap{display:none!important}
  .cluster-cards{display:block!important}

  .section{padding:12px 10px}

  /* KPI grid — allow auto height + proper text wrapping */
  .kgrid{border-spacing:4px;margin:-4px}
  .kc{padding:10px 9px 10px;min-height:auto;height:auto}
  .kc-label{font-size:10px;letter-spacing:.2px;margin-bottom:4px;font-weight:700;color:#334155}
  .kc-val{font-size:17px;margin-bottom:4px;white-space:normal;word-break:break-all}
  .kc-sub{font-size:8.5px;white-space:normal;overflow:visible;text-overflow:clip}
  .kc-trend{font-size:10px;margin-top:3px;white-space:normal}
  .kc-insight{font-size:8px;margin-top:4px;padding-top:4px}

  /* Today/Yesterday grid */
  .today-grid{border-spacing:4px;margin:-4px}
  .tc{padding:10px 9px;height:auto;min-height:85px}
  .tc-label{font-size:10px;letter-spacing:.2px;margin-bottom:4px;font-weight:700;color:#334155}
  .tc-today{font-size:17px;margin-bottom:4px;white-space:normal;word-break:break-all}
  .tc-prev{font-size:9px;white-space:normal}

  .sku-cat{font-size:11px}
  .sku-line{font-size:9.5px}
  .sku-rc{font-size:8.5px}

  .driver-chip{max-width:none;font-size:9px;width:100%}

  .watch-item{padding:10px 10px;gap:8px}
  .watch-title{font-size:11.5px}
  .watch-why{font-size:10px}

  .footer{padding:10px 10px;font-size:8px}

  /* COGS table */
  .cogs-wrap .data-table{min-width:320px;font-size:11.5px}
  .cogs-wrap .data-table th{font-size:8.5px;padding:7px 7px}
  .cogs-wrap .data-table td{padding:8px 7px;font-size:11.5px}
  .cogs-wrap .data-table td.mono{font-size:11px}

  /* Product mix table — tighten */
  .table-scroll .data-table{font-size:11px}
  .table-scroll .data-table th{font-size:8px;padding:7px 6px}
  .table-scroll .data-table td{padding:7px 6px}
}
"""
...
def build(data):
    ...
    pm_day     = calendar.monthrange(pm_last.year,pm_last.month)[1]  # full days in prior month
    pm_day_cap = min(latest.day, pm_day)   # same day-of-month as current MTD
    pm_start   = pm_last.replace(day=1).strftime('%Y-%m-01')
    pm_end     = '{}-{:02d}-{:02d}'.format(pm_last.year,pm_last.month,pm_day_cap)  # MTD-matched
    ...
    pm_ps      = fp(projects, pm_start, pm_end)            # 1-{day} vs 1-{day} MTD comparison
    pm_end_full = '{}-{:02d}-{:02d}'.format(pm_last.year, pm_last.month, pm_day)  # full prior month
    pm_full_ps  = fp(projects, pm_start, pm_end_full)   # full prior month (for projections)
    ...
    pm_full  = inject_meta(calc(pm_full_ps),  mo_onm_qhse, pm_key)
    lat = calc(lat_ps)
    prv = calc(prv_ps)
    ...
    vol_delta = 'vs {:,} {} MTD (1&#8211;{})'.format(pm['n'], prev_lbl, pm_day_cap)
    ...
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    #  PRE-COMPUTE KPI INSIGHT VALUES
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    _kpi_pace   = mtd['n'] / latest.day if latest.day > 0 else 0
    _kpi_proj   = round(_kpi_pace * 30)
    _kpi_below40= sum(1 for r in all_cl if r['curr']['gm'] < 40 and r['curr']['n'] >= MIN_ORDERS)
    _kpi_prdn   = len(price_dn)   # clusters with Rev/Wp erosion
    _kpi_gm_trend_txt = (
        '&#9650; improving vs {} MTD'.format(prev_lbl) if gm_trend >= 0.1 else
        '&#9660; softening vs {} MTD'.format(prev_lbl) if gm_trend <= -0.1 else
        'flat vs {} MTD'.format(prev_lbl))
    _kpi_rwp_txt = (
        '{} cluster{} showing Rev/Wp erosion'.format(_kpi_prdn, 's' if _kpi_prdn != 1 else '')
        if _kpi_prdn > 0 else 'Rev/Wp holding across clusters')
    _kpi_aos_txt = (
        '&#9650; larger systems &#8594; structural MMS/Cable cost pressure'
        if mtd['aos'] > pm['aos'] + 0.05 else
        '&#9660; smaller system mix vs {} MTD'.format(prev_lbl)
        if mtd['aos'] < pm['aos'] - 0.05 else
        'system size stable vs {} MTD'.format(prev_lbl))
    _kpi_b40_txt = (
        '&#9888; {} cluster{} below 40% threshold'.format(_kpi_below40, 's' if _kpi_below40 != 1 else '')
        if _kpi_below40 > 0 else '&#9989; all clusters above 40% GM')

    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    #  MTD KPI GRID (2 rows x 4)
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    def kcard(label, val, sub, vc='#111827', trend_html='', accent='#0284C7', insight=''):
        ins_html = ('<span class="kc-insight">' + insight + '</span>') if insight else ''
        return (
            ...
            '{6}'
            '</td>'
        ).format(accent, label, vc, val, sub, trend_html, ins_html)

    kpi_html = (
        '<div style="padding:4px 0"><table class="kgrid"><tr>'
        + kcard('Installations', '{:,}'.format(mtd['n']),
                'vs {:,} {} MTD (1&#8211;{})'.format(pm['n'], prev_lbl, pm_day_cap),
                trend_html=dpct(mtd['n'], pm['n']), accent='#0284C7',
                insight='~{:,} expected for {} (vs {:,} actual in {})'.format(_kpi_proj, curr_lbl[:3], pm_full['n'], prev_lbl))
        + kcard('kW Installed', '{:,.0f} kW'.format(mtd['kw']),
                'vs {:,.0f} kW {} MTD'.format(pm['kw'], prev_lbl),
                trend_html=dpct(mtd['kw'], pm['kw']),
                insight='AoS {:.2f}kW vs {:.2f}kW {} MTD'.format(mtd['aos'], pm['aos'], prev_lbl))
        + kcard('Gross Margin', '{:.2f}%'.format(mtd['gm']),
                'vs {:.2f}% {} MTD'.format(pm['gm'], prev_lbl),
                vc=gmc(mtd['gm']), trend_html=dpp(gm_trend), accent='#8B5CF6',
                insight=_kpi_b40_txt)
        + kcard('Revenue', fc(mtd['rev']),
                'vs {} {} MTD'.format(fc(pm['rev']), prev_lbl),
                trend_html=dpct(mtd['rev'], pm['rev']), accent='#059669',
                insight='&#8377;{:.2f}/Wp blended realisation'.format(mtd['rev_wp']))
        + '</tr><tr>'
        + kcard('Avg System Size', '{:.2f} kW'.format(mtd['aos']),
                'vs {:.2f} kW {} MTD'.format(pm['aos'], prev_lbl),
                trend_html=dpval(mtd['aos']-pm['aos'], 'kW'),
                insight=_kpi_aos_txt)
        + kcard('Avg Order Value', fc(mtd['aov']),
                'vs {} {} MTD'.format(fc(pm['aov']), prev_lbl),
                trend_html=dpct(mtd['aov'], pm['aov']),
                insight='&#8377;{:.2f}/Wp blended COGS this month'.format(total_cogs_pkw_c/1000))
        + kcard('Rev / Wp', '&#8377;{:.2f}'.format(mtd['rev_wp']),
                'vs &#8377;{:.2f} {} MTD'.format(pm['rev_wp'], prev_lbl),
                trend_html=dpval(rev_wp_d, '&#8377;/Wp'),
                insight=_kpi_rwp_txt)
        + kcard('Abs Gross Margin', fc(mtd['abs_gm']),
                'vs {} {} MTD'.format(fc(pm['abs_gm']), prev_lbl),
                trend_html=dpct(mtd['abs_gm'], pm['abs_gm']),
                insight='Projected full month: {}'.format(fc(mtd['abs_gm']/latest.day*30)))
        + '</tr></table>'
    )
    ...
    today_html = (
        ...
        + tcard('GM %', '{:.1f}%'.format(lat['gm']), '{:.1f}%'.format(prv['gm']),
                dpp(lat['gm']-prv['gm']) if prv['gm'] else '',
                vc=('#16A34A' if lat['gm'] >= (prv['gm'] or 0) else '#D97706'))
        + '</tr></table>'
    )
    ...
    mix_html = (
        ...
        '<th class="R">&#916;GM vs {} MTD</th><th class="R">&#916;Rev/Wp</th>'
    ...
    # ── MOBILE CARD VIEW for cluster health (visible only on mobile via CSS) ──
    def _cl_mobile_card(r, bg=''):
        c = r['curr']; p = r['prev']
        det = r['drv_det']
        rv_d = det.get('rev_wp_d', 0)
        ...
        # GM colour
        gm_pct = c['gm']
        if gm_pct >= 44: gm_bg, gm_col = '#DCFCE7', '#166534'
        elif gm_pct >= 40: gm_bg, gm_col = '#FEF9C3', '#92400E'
        else: gm_bg, gm_col = '#FEE2E2', '#B91C1C'

        # Delta colour
        if gm_d <= -0.3: d_col, d_arr = '#DC2626', '&#9660;'
        elif gm_d >= 0.3: d_col, d_arr = '#059669', '&#9650;'
        else: d_col, d_arr = '#94A3B8', '&#8212;'

        # Rev/Wp badge
        badges = ''
        if abs(rv_d) > 0.2:
            clr = '#FEE2E2' if rv_d < 0 else '#DCFCE7'
            tcl = '#B91C1C' if rv_d < 0 else '#166534'
            badges += '<span style="display:inline-block;font-size:9px;font-weight:700;padding:2px 7px;border-radius:5px;background:{};color:{};margin-right:4px;margin-bottom:4px">Rev/Wp {:+.2f}/Wp</span>'.format(clr, tcl, rv_d)
        if abs(ck_d) > 0.02:
            clr = '#FEF3C7' if ck_d > 0 else '#DCFCE7'
            tcl = '#92400E' if ck_d > 0 else '#166534'
            badges += '<span style="display:inline-block;font-size:9px;font-weight:700;padding:2px 7px;border-radius:5px;background:{};color:{};margin-right:4px;margin-bottom:4px">COGS {:+.3f}/Wp</span>'.format(clr, tcl, ck_d)

        # Insight text from insight_cell data
        insight_html = ('<b style="color:#0F172A">Root cause:</b> ' + r.get('drv_tag',''))

        bg_style = 'background:{};'.format(bg) if bg else 'background:#fff;'

        return (
            '<div style="{}border:1px solid #E5E7EB;border-left:4px solid {};'
            'border-radius:10px;padding:12px 13px;margin-bottom:8px">'

            # Header row: cluster name + GM% + delta
            '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">'
            '<span style="font-weight:800;font-size:14px;color:#0F172A">{}</span>'
            '<div style="display:flex;align-items:center;gap:6px">'
            '<span style="background:{};color:{};font-weight:800;font-size:12px;padding:3px 8px;border-radius:6px">{:.1f}%</span>'
            '<span style="color:{};font-weight:700;font-size:12px">{} {:.2f}pp</span>'
            '</div></div>'

            # Meta row: n + Rev/Wp
            '<div style="font-size:11px;color:#64748B;margin-bottom:6px">'
            'n: <b>{}</b> &nbsp;|&nbsp; Rev/Wp: <b>{:.2f}</b> / {:.2f} {} MTD'
            '</div>'
            ...
            # Insight
            '<div style="font-size:11px;color:#334155;line-height:1.7;word-break:break-word">'
            '{}'
            '</div>'
            ...
            bg_style,
            '#DC2626' if gm_d <= -0.3 else ('#059669' if gm_d >= 0.3 else '#94A3B8'),
            r['cluster'],
            gm_bg, gm_col, gm_pct,
            d_col, d_arr, abs(gm_d),
            c['n'], c['rev_wp'], p['rev_wp'] if p['n'] else 0, prev_lbl,
            badges,
            insight_html
        )

    def _mob_section_banner(icon, label, bg_color, text_color, border_color, dot_color):
        """Centered full-width section divider for mobile cluster cards."""
        return (
            '<div style="'
            'text-align:center;margin:14px 0 10px;position:relative;'
            '">'
            # Side lines via border trick
            '<div style="'
            ...
            '</span>'
            '</div>'
        ).format(bg=bg_color, txt=text_color, border=border_color, icon=icon, label=label)

    mob_cards_html = ''
    if declining:
        mob_cards_html += _mob_section_banner(
            '&#9660;', 'Declining vs {} &mdash; Needs Attention'.format(prev_lbl),
            '#FEE2E2', '#B91C1C', '#FECACA', '#DC2626')
        mob_cards_html += ''.join(_cl_mobile_card(r,'#FFFBFB') for r in declining)
    if improving:
        mob_cards_html += _mob_section_banner(
            '&#9650;', 'Improving vs {}'.format(prev_lbl),
            '#DCFCE7', '#166534', '#A7F3D0', '#059669')
        mob_cards_html += ''.join(_cl_mobile_card(r,'#F9FFFA') for r in improving)
    if stable_cl:
        mob_cards_html += _mob_section_banner(
            '&#8594;', 'Stable (within &plusmn;0.3pp)',
            '#F1F5F9', '#475569', '#CBD5E1', '#94A3B8')
        mob_cards_html += ''.join(_cl_mobile_card(r) for r in stable_cl)
    if nascent:
        mob_cards_html += _mob_section_banner(
            '&#9733;', 'New / Growing Clusters',
            '#EDE9FE', '#5B21B6', '#DDD6FE', '#7C3AED')
        mob_cards_html += ''.join(_cl_mobile_card(r,'#FAF5FF') for r in nascent)

    cluster_mobile_cards_html = '<div class="cluster-cards">{}</div>'.format(mob_cards_html) if mob_cards_html else ''
    # Combine: desktop table + mobile cards (CSS controls which is shown)
    cl_html = cl_html + cluster_mobile_cards_html
    ...
    if latest.day > 1:
        ...
        runrate_bar = (
            ...
            'line-height:2;'
            '">'
            '&#128200; <b>Daily run-rate for {curr_lbl_short}:</b> {pace_int} installations/day'
            ...
            pace_int=int(_pace), proj=_proj,
            pm_n=pm_full['n'], prev_lbl=prev_lbl,
...
<style>''' + CSS + '''</style></head><body><div class="page">''' + ''.join([
        ...
        runrate_bar,
        section('MTD Dashboard', '{} MTD vs {} MTD (1&#8211;{} days)'.format(curr_lbl[:3], prev_lbl, pm_day_cap), kpi_html),
        section('Yesterday at a Glance', '{} vs {}'.format(lat_lbl, prv_lbl), today_html),
        section('Product Mix', '{} MTD vs {} MTD &mdash; installs, GM%, Rev/Wp'.format(curr_lbl, prev_lbl), mix_html),
        section('COGS Analysis', '{} MTD vs {} MTD &#8212; SKU-level root cause'.format(curr_lbl, prev_lbl), cogs_html),
